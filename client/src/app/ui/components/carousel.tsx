...
export default function Carousel({ id, title, data, isCategory = false, isProduct = false, isAllProducts = false, isAllCategories = false, productCardCls = "", token = null }: CarouselProps) {
  // TODO nanti untuk desc dan href boleh null DAN MUNGKIN UNTUK TIPE DATANYA BISA DIBIKININI INTERFACE  
  // TODO: aku belum nambahin kasus jika jumlah displayed data kurang atau pas dari ketentuannya, itu button prev n next harusnya hidden ae dulu
  const displays: any = []
  ...
  const itemDisplayed = isCategory ? [3, 2, 2, 1, 1] : isProduct ? [4, 3, 3, 2, 2] : isAllCategories ? [2, 2, 2, 2, 1] : [5, 4, 3, 2, 2]
  ...
  const onClickSlider = ((isNext: boolean) => {
    // TODO: size dari carousel ketika ukuran layar diubah secara responsive, posisi gambar nggak masih mengikuti size yg ditentukan di awal, coba deh tes klik next berapa kali gitu lalu size layar diubah secra responsif (drag kanan kiri gitu)
    if (carouselContainer?.children[0] instanceof HTMLElement && carouselContainer?.children[1] instanceof HTMLElement && carouselChildrenCounts) {
      console.log(carouselContainer?.children, 'carouselContainer?.children[1]');
      ...
      if (isNext) {
        ...
        if (carouselChildrenCounts == (mediaXl.matches ? itemDisplayed[0] : mediaLg.matches ? itemDisplayed[1] : mediaMd.matches ? itemDisplayed[2] : mediaSm.matches ? itemDisplayed[3] : itemDisplayed[4])) {
          setIsEndLimit(true)
  ...
  return (
    ...
    <>
      <div className="lg:mb-6 sm:mb-[18px] mb-3 flex w-full">
      ...
      <div>
        <div className={clsx("mx-auto", { "overflow-x-hidden": isCategory || isProduct || isAllProducts, "overflow-x-scroll": isAllCategories, "carousel-container relative": isCategory || isProduct || isAllProducts || isAllCategories, "xl:h-[274px] lg:h-[284px] md:h-[220px] xs:h-[324px] h-[214px]": isCategory, "xl:h-[368px] lg:h-[368px] md:h-[308px] sm:h-[344px] xs:h-[274px] h-[254px]": isProduct, "xl:h-[137px] md:h-[142px] xs:h-[162px] h-[117px]": isAllProducts, "xl:h-[338px] lg:h-[300px] md:h-[250px] sm:h-[188px] xs:h-[316px] h-[200px] lg:mt-10 sm:mt-[30px] mt-5": isAllCategories })}>
          <div id={`carouselContainer-${id}`} className={clsx({ "absolute flex h-full": isCategory || isProduct || isAllProducts || isAllCategories, "md:gap-4 gap-2": isCategory || isProduct, "grid lg:gap-[50px] sm:gap-[30px] xs:gap-[25px] gap-5 lg:grid-cols-3 grid-cols-2": !isCategory && !isProduct && !isAllProducts && !isAllCategories, "flex lg:gap-x-3 gap-x-1.5": isAllProducts, "md:gap-5 gap-2.5": isAllCategories })}>
